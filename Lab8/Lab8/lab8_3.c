...
* Description: Create a sin wave from 0 to 2.5v at a frequency of 1567.98 Hz using events and DMA
*/
...
void tcc0_init(void)
{
	// initialize the timer counter to 1567.98 Hz
	TCC0.PER = 79;
...
void DMA_init(void)
{
	...
	DMA.CH0.TRFCNT = (uint16_t)(sizeof(sin_tab));
	// repeat block transfer forever
	DMA.CH0.REPCNT = 0;

	// set the source address to the sin_tab address
	DMA.CH0.SRCADDR0 = (uint8_t)((uintptr_t)sin_tab);
	DMA.CH0.SRCADDR1 = (uint8_t)(((uintptr_t)sin_tab) >> 8);
	DMA.CH0.SRCADDR2 = (uint8_t)(((uint32_t)(uintptr_t)sin_tab) >> 16);
	// set the destination address to DAC_CH0DATA
	DMA.CH0.DESTADDR0 = (uint8_t)((uintptr_t)&DACA.CH0DATAL);
	DMA.CH0.DESTADDR1 = (uint8_t)((uintptr_t)&DACA.CH0DATAL >> 8);
	DMA.CH0.DESTADDR2 = (uint8_t)((uint32_t)(uintptr_t)&DACA.CH0DATAL >> 16);
