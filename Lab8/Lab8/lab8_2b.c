...
* Description: Create a sin wave from 0 to 2.5v at a frequency of 1567.98 Hz using events
*/
...
uint16_t *sin_tab_pointer = &sin_tab;
// create a pointer to the end of table
uint16_t *sin_eot = 0x2200;
...
int main(void)
{
	...
	while (1)
	{
		// wait on until event system indicates ready to transfer
		while(!(DACA.STATUS & DAC_CH0DRE_bm)){};
		// output data from table
		DACA.CH0DATA = *sin_tab_pointer;
		// increment table pointer
		sin_tab_pointer++;
		// check if at end of table
		if(sin_tab_pointer >= sin_eot)
		{
			// set pointer to the start of the table
			sin_tab_pointer = &sin_tab;
		}
	}	
	return 0;
}
...
void DAC_init(void)
{
	...
	// set event channel 0 to trigger conversion
